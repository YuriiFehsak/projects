...
async def select_db(message: types.Message):
        ...
        await bot.send_message(message.from_user.id, f"Дані от такі:  Ccy: '{result[0]}', buy_price: {result[1]}, sale_price: {result[2]}, time: {result[3]}")
        connect.commit()
        cursor.close()
        connect.close()

@dp.message_handler(commands=['delete'],)
async def delete_db(message: types.Message, key):
